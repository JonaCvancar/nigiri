...
int main() {
  ...
  unixtime_t begin_d = unixtime_t{vbb_period_oa().from_};
  unixtime_t end_d = unixtime_t{vbb_period_oa().from_};
  ...
  */
  ...
  int days_count = vbb_period_oa().size().count();
  std::uniform_int_distribution<> dis_days(0, static_cast<int>(days_count));

  while(true) {
    std::ifstream infile("/home/jona/uni/thesis/results/berlin/test_stops.txt");
    std::vector<std::string> file_content;

    // Check if the file can be opened
    if (!infile.is_open()) {
      std::cerr << "Failed to open file: " << "/home/jona/uni/thesis/results/berlin/test_stops.txt" << std::endl;
      return 1;
    }
    ...
    if (infile_temp.is_open() && infile_temp.peek() == std::ifstream::traits_type::eof()) {
      infile_temp.close();
    ...
    if(!file_content.empty()) {
      ...
      std::ofstream outfile_stops("/home/jona/uni/thesis/results/berlin/test_stops.txt");
