...
int main() {
  ...
  auto const locations = routing::tripbased::onetoall::get_locations((vbb_dir_oa.path().string() + "/stops.txt"), ',');
  std::cout << "location length: " << locations.size() << "\n";
  std::cout << "First stop:" << locations[0] << "\n";

  // run preprocessing
  transfer_set ts;
  build_transfer_set(tt, ts);

  unixtime_t begin_d = unixtime_t{sys_days{December / 9 / 2022}};
  unixtime_t end_d = unixtime_t{sys_days{December / 9 / 2022}};
        ...
        for (int j = 0; j < 250; j++) {
          ...
          for (int k = 0; k < 20; k++) {
            ...
            tripbased_onetoall_query(
                ...
                interval{unixtime_t{sys_days{December / 9 / 2022}} + days{day} +
                         hours{time},
                         unixtime_t{sys_days{December / 9 / 2022}} + days{day} +
